/**
 * Builds, told the way an FDE reports them: the problem, the constraint that
 * shaped the design, what actually shipped, and the outcome with a number
 * attached. `problem`/`solution`/`impact` are kept for the shorter surfaces
 * that only need one line each.
 *
 * Honesty rule: every entry says where it was built (`builtAs`), and nothing
 * here may imply a customer, a client team, or adoption that didn't happen.
 */
export type Deployment = {
  ...
  solution: string;
  impact: string;
  /** "Personal build" or "Built at <company>" — shown on the card and dossier. */
  builtAs: string;
  /** The problem space the build addresses. */
  context: string;
  /** My scope of ownership on the build. */
  role: string;
  ...
  /** Systems this build had to plug into. */
  surfaces: string[];
  tags: string[];
  github: string;
  demo: string;
...
export const featuredProjects: Deployment[] = [
  {
    ...
    shortDesc:
      "HR platform with compensation analytics over a generated 10,000-employee dataset — Rails 7 API + Next.js 16.",
    description:
      "A production-quality, full-stack salary management platform built for HR managers. Handles a seeded dataset of 10,000 employee records with full CRUD, pagination, search, and 5 deep analytical views: salary by country, by job title, department headcount, tenure distribution, and top-paying roles.",
    problem:
    ...
    impact:
      "10,000-record generated dataset · Sub-200ms queries via indexed PostgreSQL · 13+ RSpec model tests · Production-ready architecture",
    // TODO(confirm): personal build, or built for an employer/client?
    builtAs: "Personal build",
    context:
      "HR teams that run compensation out of spreadsheets get no view of salary equity, headcount distribution, or tenure across departments and countries. I built this to solve that at realistic volume. All employee data is generated by the seed script — there is no real payroll data in it.",
    role: "Sole engineer — architecture, build, deploy",
    constraint:
      "Compensation data is sensitive and high-volume. I wanted the analytics fast enough to explore live, and every rule that touches pay to be testable — so caching tricks and denormalised shortcuts were off the table.",
    shipped: [
      ...
      "Indexed PostgreSQL schema plus a seed system that generates the full 10,000-employee demo dataset in ~2.4s",
  ...
  },
  {
    ...
    builtAs: "Personal build",
    context:
      "Most e-commerce starters lean on a hosted commerce layer — every customisation means fighting the platform, and every deploy means coordinating several vendors. I built this to see how far one self-contained app could go.",
  ...
  },
  {
    ...
    // TODO(confirm): personal build, or built for an employer/client? If it
    // was company work, set builtAs to "Built at <company>" and drop the repo
    // links if they shouldn't be public.
    builtAs: "Personal build",
    context:
      "When Iterable has no idea what users are doing inside the product, campaigns run blind: onboarding and re-engagement fire on calendar time instead of behaviour.",
    role: "Sole engineer — Rails integration service",
    constraint:
      "Designed to drop into an existing Rails app. The integration must not change application behaviour, must not block a user request, and must not take the app down if Iterable is slow or unavailable. Fail-open is the hard requirement.",
    shipped: [
      ...
      "A vendor-agnostic service pattern, reusable for the next integration",
    ],
    outcome: [
...
export const blogPosts: BlogPost[] = [
  ...
  },
  {
    ...
    content: [
      ...
      {
        type: "paragraph",
        text: "On the salary management platform I built this way, the Rails API handled a seeded 10,000-record employee dataset with indexed sub-200ms queries while the Next.js frontend rendered five analytical dashboards — and I could tune either side in isolation without fear of breaking the other.",
      },
